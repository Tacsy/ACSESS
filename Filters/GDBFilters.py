...
newfilt.SetExceptions([Chem.MolFromSmarts("[C&X3]=CNC=[O,N]"),
                       ...
                       Chem.MolFromSmarts("[C,N,O]1C([C,O])=[CH1]NC([C,O])=C1([C,O])"), #0100,0010
...
newfilt.SetExceptions([Chem.MolFromSmarts("O=C[O&H1]"),
...
newfilt.SetExceptions([Chem.MolFromSmarts("Fc([a])=[N,O]")]) #Should [n,o] be aromatic or not?
...
newfilt.SetExceptions([Chem.MolFromSmarts("[#6]N([#6])N=[#6]"),
...
newfilt.SetExceptions([Chem.MolFromSmarts("[#6]=NO[C,S]"),
                          Chem.MolFromSmarts("[#6]=N[O&H1]"),
                          Chem.MolFromSmarts("[N,O,S]=[C,S]NO[C,S]"),
                          Chem.MolFromSmarts("[N,O,S]=CN[O&H]"),
                          nitros])
...
newfilt.SetExceptions([Chem.MolFromSmarts("O=[C,S]1**[C,S](O1)=O"),
...
newfilt=NewPatternFilter('Ortho1') #Note that this differs from what they said
...
for filtname in ['aminal','C=N','Enamine','HetHet_NN','HetHet_OO',
                 'HetHet_NO','Ortho1','Ortho2','HetHet_SS','Enol','HetHet_NS',
                 'HetHet_OS1','HetHet_OS3','HetHet_OS2',
                 'HetHet_Aromatic','HetHetHet','Non-aromatic nitro',
                 'Non-aromatic halogen','Beta keto carboxylate',
                 'Orthoester']:
