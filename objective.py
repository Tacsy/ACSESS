...
def SelectFittest(newpool, subsetSize, gen):
    ...
    if len(newpool) > subsetSize + TakeFittest:
        ...
        pool = newpool[
            TakeFittest:]  ###at this point pool is updated so that only compounds satisfying fitness stay
        ...
        for ipick in picks:
            ...
            distsqr = np.ma.masked_array(
                distsqr, mask=array_or(targetmask, neighbor_pick_mask))
            if distsqr.count() > 0:
